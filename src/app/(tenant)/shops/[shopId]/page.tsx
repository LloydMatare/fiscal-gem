...
import { db } from "@/db";
import { shops, agents } from "@/db/schema";
...
export default async function ShopDetailPage({
  ...
  const shop = await db.query.shops.findFirst({
    where: and(eq(shops.id, shopId), eq(shops.clientId, client.id)),
  });
  if (!shop) notFound();
  ...
  return (
    <div>
      <PageHeader
        title={shop.name}
        description={`${shop.city || ""} ${shop.address || ""}`.trim() || "Shop details"}
        breadcrumbs={[
          { label: "Shops", href: "/shops" },
          { label: shop.name, href: `/shops/${shopId}` },
        ]}
      />
